...
import yara
from getFiles import getListOfFiles
import pdb
...
def scan_yara(file):
      virus_found = False
      # pdb.set_trace()
      # for rule_file in fileList:
      #       if ".yar" in rule_file and "index" not in rule_file:             
      #             rules = yara.compile(filepath=rule_file)
      #             matches = rules.match(filepath=file)
      #             if len(matches)==0:
      #                   virus_found = False
      #             else: 
      #                   virus_found = True
      ...
      rules = yara.compile(filepath="./rules/myRules/rule.yar")
      matches = rules.match(filepath=file)
      if len(matches) == 0:
            virus_found = False
      else:
